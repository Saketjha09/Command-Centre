...
func RegisterRoutes(mux *http.ServeMux, pool *pgxpool.Pool, cfg *config.Config) {
	...
	mux.Handle("GET /api/v1/brands",
		middleware.Authenticate(cfg)(HandleListBrands(pool)))
	...
	mux.Handle("POST /api/v1/brands",
		middleware.Authenticate(cfg)(adminOnly(HandleCreateBrand(pool))))

	mux.Handle("DELETE /api/v1/brands/{id}",
		middleware.Authenticate(cfg)(adminOnly(HandleDeleteBrand(pool))))
}
